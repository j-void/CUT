...
class N_UpsampleBlock(nn.Module):
    def __init__(self, scale_factor, input_dim, output_dim, activ, norm, num_classes):
        super().__init__()
        self.upsample = Upsample2(scale_factor=scale_factor)
        self.conv = N_Conv2dBlock(
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    elif netG == 'resnet_seg_film':
        n_blocks = 9
        net = N_Resnet(input_nc, output_nc, num_downs=2, n_res=n_blocks - 4, ngf=ngf, norm='film', nl_layer='relu', num_classes=opt.num_classes)
    else:
...
class N_Resnet(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, n_res, num_classes, ngf=64,
                 norm=None, nl_layer=None):
        super(N_Resnet, self).__init__()
        n_downsample = num_downs
        pad_type = 'reflect'
        self.enc_content = ContentEncoder(n_downsample, n_res, input_nc, ngf, 'inst', nl_layer, pad_type=pad_type)
        self.dec = N_Decoder(n_downsample, n_res, self.enc_content.output_dim, output_nc, norm=norm, activ=nl_layer, pad_type=pad_type, nz=0, num_classes=num_classes)


    def decode(self, content, seg_onehot):
        d = self.dec(content, seg_onehot)
        return d
...
class S_Decoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, norm='batch', activ='relu', pad_type='zero', nz=0, num_classes=None):
...
class N_Decoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, norm='batch', activ='relu', pad_type='zero', nz=0, num_classes=None):
        super(N_Decoder, self).__init__()

        blocks = []
        self.in_channels_list = []

        # 1) ResBlocks as first logical block
        blocks.append(N_ResBlocks(n_res, dim, norm, activ, pad_type=pad_type, nz=nz, num_classes=num_classes))
        self.in_channels_list.append(dim)
        ...
        for i in range(n_upsample):
            ...
            self.in_channels_list.append(input_dim)
            up_block = N_UpsampleBlock(2, input_dim, dim // 2, activ, norm, num_classes)
            blocks.append(up_block)
            dim = dim // 2

        # 3) Final conv as last logical block
        final_block = N_Conv2dBlock(dim, output_dim, 7, 1, 3, norm='none', activation='tanh', pad_type='reflect', num_classes=num_classes)
        blocks.append(final_block)
        self.in_channels_list.append(dim)
    ...
    def forward(self, x, seg_onehot=None):
        for block in self.blocks:
            x = block(x, seg_onehot)
        return x
...
class FiLM(nn.Module):
    def __init__(self, num_classes, num_features):
        super().__init__()
        self.mlp = nn.Conv2d(num_classes, num_features * 2, kernel_size=3, padding=1)
        
 
        ### need to move this initialization
        nn.init.zeros_(self.mlp.weight)
        nn.init.zeros_(self.mlp.bias)

    def forward(self, x, seg):
        seg = F.interpolate(seg, size=x.shape[2:], mode='nearest')
        gamma_beta = self.mlp(seg)
...
class N_ResBlock(nn.Module):
    def __init__(self, dim, norm='inst', activation='relu', pad_type='zero', nz=0, num_classes=None):
        super().__init__()

        self.blocks = nn.ModuleList([
            N_Conv2dBlock(dim + nz, dim, 3, 1, 1,
                          norm=norm, activation=activation,
                          pad_type=pad_type, num_classes=num_classes),
            N_Conv2dBlock(dim, dim + nz, 3, 1, 1,
...
class N_ResBlocks(nn.Module):
    def __init__(self, num_blocks, dim, norm='inst', activation='relu',
                 pad_type='zero', nz=0, num_classes=None):
        super().__init__()
        self.blocks = nn.ModuleList([
            N_ResBlock(dim, norm=norm, activation=activation,
                       pad_type=pad_type, nz=nz, num_classes=num_classes)
...
class N_Conv2dBlock(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size, stride,
                 padding=0, norm='none', activation='relu', pad_type='zero', num_classes=None):
        super(N_Conv2dBlock, self).__init__()
        self.use_bias = True
        # initialize padding
        if pad_type == 'reflect':
            self.pad = nn.ReflectionPad2d(padding)
        ...
        if norm == 'batch':
            ...
            self.norm = FiLM(num_classes=num_classes, num_features=norm_dim)
        else:
            assert 0, "Unsupported normalization: {}".format(norm)
